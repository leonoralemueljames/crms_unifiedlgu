...
<html lang="en">
  <head>
    ...
    <title>Compliance Monitoring - CRMS PORTAL</title>
...
$result_active = $conn->query("SELECT COUNT(*) as active_users FROM admin_accounts WHERE status = 'active'");
...
$result_blocked = $conn->query("SELECT COUNT(*) as blocked_users FROM admin_accounts WHERE status = 'blocked'");
  ...
  <div class="card  mb-4">
    <div class="card-body">
      <div class="row align-items-center">
        <div class="col">
          <small class="text-muted-show mb-1">Compliance Rate</small>
...
?> %</h3>
        ...
        <div class="col-4 text-right">
          <span class="sparkline "><img src="../../../assets/images/evaluate.png"  width="70"></span>
        </div>
      </div> <!-- /. row -->
    </div> <!-- /. card-body -->
  </div> <!-- /. card -->
...
<div class="col-md-4">
  <div class="card  mb-4">
    <div class="card-body">
      <div class="row align-items-center">
        <div class="col">
          <small class="text-muted-show mb-1">Employee Completion</small>
          <h3 class="card-title" style="font-size:32px;"><?php
echo$active_users;
?></h3>
        ...
        <div class="col-4 text-right">
          <span class="sparkline"><img src="../../../assets/images/employee.png" width="50"></span>
        </div>
      </div> <!-- /. row -->
    </div> <!-- /. card-body -->
  </div> <!-- /. card -->
...
<div class="col-md-4">
  <div class="card mb-4">
    <div class="card-body">
      <div class="row align-items-center">
        <div class="col">
          <small class="text-muted-show mb-1">Compliance Fail</small>
          <h3 class="card-title" style="font-size:32px;">1</h3>
                                  
        </div>
        <div class="col-4 text-right">
          <span class="sparkline"><img src="../../../assets/images/audit.png" width="50"></span>
        </div>
                      ...
                      <!-- table -->
  <table class="table datatables table-striped table-hover" id="dataTable-1">
...
<thead style="padding:20px">
        ...
        <th>Name</th>
          <th>Department</th>
          <th>Compliance Training Completed</th>
...
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
    ...
    echo "<td style='max-width:200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;'>" . $row["department"]. "</td>";
    echo "<td>". substr($row["created_at"], 0, 20) . "</td>";
    ...
    if ($row["status"] == "active") {
      echo "<td style='text-align:center;' class='badge-status badge-success-active'>" . $row["status"] . "</td>";
  } else {
    ...
    echo "<td>
          <button type='button' class='list-button btn-primary' data-toggle='modal' data-target='#editModal" . $row["id"] . "'><i class='fa-solid fa-edit fe-14' data-bs-toggle='tooltip' title='Update'></i></button>&nbsp;&nbsp;&nbsp;&nbsp;
     
          ";
       
            echo "<button type='button' class='list-button btn-danger' data-toggle='modal' data-target='#deleteModal" . $row["id"] . "'><i class='fe fe-trash fe-14'></i></button> ";
          echo "</td>";
          echo "</tr>";

          // Modals
          echo "<div id='deleteModal" . $row["id"] . "' class='modal fade' role='dialog'>";
          echo "<div class='modal-dialog modal-sm modal-dialog-centered'>";
          echo "<div class='modal-content'>";
          echo "<div class='modal-header'>";
          echo "<h5 class='modal-title'>Delete Admin User</h5>";
          echo "<button type='button' class='close' data-dismiss='modal'>&times;</button>";
          echo "</div>";
          echo "<div class='modal-body text-center p-4'>";
          echo "<p class='font-weight-bold mb-4'>Are you sure you want to delete the user <span class='text-danger'>" . $row["firstname"] . " " . $row["lastname"] . "</span> with ID " . $row["id"] . "?</p>";
          echo "<p class='text-muted'>This action cannot be undone.</p>"; // Updated message
          echo "<form action='' method='post'>";
          echo "<input type='hidden' name='id' value='" . $row["id"] . "'>";
          echo "<input type='hidden' name='delete' value='delete'>";
          echo "<div class='d-flex justify-content-center mt-4'>";
          echo "<button type='submit' class='btn btn-danger btn-lg mr-2'>Delete</button>"; // Changed button text to 'Delete'
          ...
          if (isset($_POST['delete'])) {
            ...
            if ($query == true) {
                ...
                echo "<script>window.location.href='../../admin/account_mgmt/admin-account';</script>";
              ...
              if (isset($_POST['submit'])) {
                ...
                $sql = "UPDATE `admin_accounts` SET  `firstname`='$firstname' , `lastname`='$lastname', `email`='$email' WHERE id='$id' ";
...
$firstname = $lastname = $email = $password = $username = $role = $confirm_password = "";
$firstnameerr = $lastnameerr = $emailerr = $usernameerr = $role_err = $passworderr = "";
...
if (empty($_POST["role"])) {
  ...
  if (empty($firstnameerr) && empty($lastnameerr) && empty($emailerr) && empty($usernnameerr) && empty($role_err) && empty($passworderr) && empty($confirm_passworderr)) {
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

    $sql = "INSERT INTO admin_accounts (firstname, lastname, email, username, role, password) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssss", $firstname, $lastname, $email, $username, $role, $hashedPassword);

    if ($stmt->execute()) {

      include ('../../mail/credentials'); 
